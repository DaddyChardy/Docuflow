...
export const DocumentGenerator: React.FC<DocumentGeneratorProps> = ({ user, initialType = DocumentType.ACTIVITY_PROPOSAL, initialDoc, onBack }) => {
  ...
  // Can Edit if:
  // 1. User has 'edit' permission for this specific DocType
  // AND (It's a New Document OR User is Owner OR It's Shared with Dept)
  ...
  const toggleLiveAgent = async () => {
    if (isLiveActive) {
      ...
      const session = new LiveSession((gatheredData) => {
        // Callback when form data is gathered by voice

        // 1. Update the local UI state so the user sees the gathered data
        if (gatheredData && typeof gatheredData === 'object') {
          setFormData(prev => ({ ...prev, ...gatheredData }));
        }

        // 2. Disconnect the Voice Agent (it finished its job)
        if (liveSessionRef.current) {
          liveSessionRef.current.disconnect();
          liveSessionRef.current = null;
        }
        ...
        // 3. IMMEDIATELY trigger the standard Document Generation using the unified RAG + Template pipeline
